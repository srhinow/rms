...
{
    $GLOBALS['TL_DCA']['tl_news']['config']['onload_callback'][] = array('ReleaseManagementSystem','addRMFields'); 
    $GLOBALS['TL_DCA']['tl_news']['list']['operations']['toggle']['button_callback'] = array('tl_rms_news','toggleIcon');
    $GLOBALS['TL_DCA']['tl_news']['list']['operations']['showPreview'] = array                 
			(
				...
				'button_callback' => array('tl_rms_news','checkPreviewIcon')
			);
 ...
 * @package    Controller
 */
class tl_rms_news extends Backend
{
    ...
    public function checkPreviewIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        $previewLink = $this->ReleaseManagementSystem->getPreviewLink($row['id'],'tl_news');    

	...
	*/
	public function previewContentElement(Database_Result $objElement, $strBuffer)
	{
	    if($this->Input->get('do') == 'preview' || $this->Input->get('do') == 'article')
	    {
                ...
                {
		    case 'news':
			//only contentelement where ist detail-module in properties
			if($objElement->module == $GLOBALS['TL_CONFIG']['rms_prevmodule_news'])
			{
			    $table = 'tl_news';
			    $entryObj = $this->Database->prepare('SELECT * FROM `tl_news` WHERE `id`=? OR `alias`=?')
							  ->limit(1)
							  ->execute($this->Input->get('items'), $this->Input->get('items'));
			    $id = $entryObj->id;
// 			    $strClass = $this->findFrontendModule($objElement->type);
			    $typePrefix = 'mod_';			    
			}
		    break;
		    case 'newsletter':
			//only contentelement where ist detail-module in properties
			if($objElement->module == $GLOBALS['TL_CONFIG']['rms_prevmodule_newsletter'])
			{		    
			   $table = 'tl_newsletter';
			   $entryObj = $this->Database->prepare('SELECT * FROM `tl_newsletter` WHERE `id`=? OR `alias`=?')
						      ->limit(1)
						      ->execute($this->Input->get('items'), $this->Input->get('items'));		       		       
			    $id = $entryObj->id;
// 			    $strClass = $this->findFrontendModule($objElement->type);
			...
			//only contentelement where ist detail-module in properties
			if($objElement->module == $GLOBALS['TL_CONFIG']['rms_prevmodule_calendar_events'])
			{		    
			   $table = 'tl_calendar_events'; 		
			   $entryObj = $this->Database->prepare('SELECT * FROM `tl_calendar_events` WHERE `id`=? OR `alias`=?')
						      ->limit(1)
...
// 		        $strClass = $this->findContentElement($objElement->type);
			...
			{
			    case 'mod_':
				$modObj = $this->Database->prepare("SELECT * FROM tl_module WHERE id=?")
										 ->limit(1)
										 ->execute($objElement->module);
				$objRow = $this->overwriteDbObj($modObj, deserialize($objStoredData->data));	
// 	    			$objRow = $modObj;
// 	    			print "<pre>";
// 	    			print_r($modObj->row());
// 	    			print "</pre>";
// 	    			print "<pre>";
// // 	    			print_r($objRow->row());
// 	    			print_r(deserialize($objStoredData->data));
// 	    			print "</pre>";
				$objRow->typePrefix = $typePrefix;
				$objRow->published = 1;	
												 				
				$strClass = $this->findFrontendModule($modObj->type);
// 				print $strClass.' '.$modObj->type;
				$objModule = new $strClass($objRow, 'main');

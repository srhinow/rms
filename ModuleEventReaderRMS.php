 ...
 * @package    Controller
 */
class ModuleEventReaderRMS extends ModuleEventReader
{
	...
	protected function compile()
	{
		...
		// Get the current event as preview or standart
		if($this->Input->get('do') == 'preview')
		{
		    $entryObj  = $this->Database->prepare("SELECT *, author AS authorId, (SELECT title FROM tl_calendar WHERE tl_calendar.id=tl_calendar_events.pid) AS calendar, (SELECT name FROM tl_user WHERE id=author) author FROM tl_calendar_events WHERE pid IN(" . implode(',', array_map('intval', $this->cal_calendar)) . ") AND (id=? OR alias=?)")
					      ->limit(1)
		    ...
		    $rmsObj = $this->Database->prepare("SELECT `data` FROM `tl_rms` WHERE `ref_id`=? AND `ref_table`=?")
					->execute((int) $entryObj->id, 'tl_calendar_events');

		    $objEvent = $this->overwriteDbObj($entryObj, deserialize($rmsObj->data));		
		    $objEvent->reset();		

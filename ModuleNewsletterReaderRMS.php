...
class ModuleNewsletterReaderRMS extends ModuleNewsletterReader
{
	...
	protected function compile()
	{
		...
		{
		    $entryObj  = $this->Database->prepare("SELECT *, (SELECT title FROM tl_newsletter_channel WHERE tl_newsletter_channel.id=tl_newsletter.pid) AS channel FROM tl_newsletter WHERE pid IN(" . implode(',', array_map('intval', $this->nl_channels)) . ") AND (id=? OR alias=?)" )
					      ->limit(1)
		    ...
		    $rmsObj = $this->Database->prepare("SELECT `data` FROM `tl_rms` WHERE `ref_id`=? AND `ref_table`=?")
					->execute((int) $entryObj->id, 'tl_newsletter');

		    $objNewsletter = $this->overwriteDbObj($entryObj, deserialize($rmsObj->data));		
		    $objNewsletter->reset();		
